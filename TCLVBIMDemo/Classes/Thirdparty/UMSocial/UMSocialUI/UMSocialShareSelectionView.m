...
#define UMSocial_Max_Row_Count 1 //最大行数（计算高度时会用到）
...
{
    NSInteger rowCount = ceilf((float)paltformArr.count/UMSocial_Item_Count_PerRow);
...
- (void)show
{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        ...
        CGRect cancelButtonFrame = self.cancelButton.frame;
        cancelButtonFrame.size.width = self.frame.size.width;
        cancelButtonFrame.origin.y = self.superview.frame.size.height-cancelButtonFrame.size.height - 5;
        self.cancelButton.frame = cancelButtonFrame;
        self.backgroundGrayView.alpha = UMSocial_BgGray_View_Alpha;
